...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	...
	POINT_3D anchors[5][4]; // siatka 4x4 punktów kontrolnych (anchor)
...
int divs = 8; // number of interpolation (distribution of polygon)
...
void initBezier(void) {
	...
	mybezier.anchors[4][0] = makePoint(-1.75, 0.75, -0.50);

	mybezier.anchors[4][1] = makePoint(-1.25, 0.75, -1.00);

	mybezier.anchors[4][2] = makePoint(1.25, 0.75, -1.00);

	mybezier.anchors[4][3] = makePoint(1.75, 0.75, -0.50);
...

{
	...
	GLboolean error = load_targa("water_ocean_20150320_1895014205.tga", width_texture, height_texture, format, type, Ptr_mybezier_texture);
